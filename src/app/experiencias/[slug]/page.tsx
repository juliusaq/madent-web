...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${formattedTitle} | Experiencias`,
    description: `Conoce la experiencia de ${formattedTitle} con MADENT. Descubre cómo están transformando su práctica dental.`,
    alternates: {
      canonical: `/experiencias/${slug}`,
    },
  };
}

export default async function ExperienceStoryPage({ params }: PageProps) {
  const { slug } = await params;
      ...
      <main className="flex-1 py-24 sm:py-32 flex flex-col items-center justify-center">
        <div className="max-w-xl mx-auto px-6 sm:px-8 text-center flex flex-col items-center">
          ...
          <p className="text-text text-sm sm:text-base leading-relaxed mb-10">
            Esta historia se encuentra en preparación. Muy pronto podrás conocer la experiencia completa de <strong>{formattedTitle}</strong> y cómo están adoptando la plataforma para mejorar el orden y la experiencia de sus pacientes.
          </p>
